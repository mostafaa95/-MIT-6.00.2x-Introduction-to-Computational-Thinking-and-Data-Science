...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        """
        self.pos = pos
        self.pos.x = math.floor(self.pos.getX())
        self.pos.y = math.floor(self.pos.getY())
        if self.pos.__str__() not in self.cleaned:
            self.cleaned.append(self.pos.__str__())
    ...
    def isTileCleaned(self, m, n):
        ...
        self.m = m
        self.n = n
        return Position.__str__(Position(self.m, self.n)) in self.cleaned
    ...
    def isPositionInRoom(self, pos):
        ...
        hh = self.pos.getX()
        bb = self.pos.getY()
        if (hh >= 0 and hh < self.width) and (bb >= 0 and bb < self.height):
            return True
        else:
            return False
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        posi = Position.getNewPosition(self.getRobotPosition(), self.getRobotDirection(), self.speed)
        while(not(self.room.isPositionInRoom(posi))):
            self.direction = random.randint(0, 360)
            self.setRobotDirection(self.direction)
            posi = Position.getNewPosition(self.getRobotPosition(), self.getRobotDirection(), self.speed)
        self.setRobotPosition(posi)
